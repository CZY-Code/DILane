...
def culane_metric_v2(pred, #原图基准上的所有（x,y）车道线
                  anno,
                  width=30,
                  iou_thresholds=[0.5],
                  official=True,
...
def load_culane_data(data_dir, file_list_path):
    ...
    data = []
    for path in filepaths: #对每张图片的数据进行处理
        img_data = load_culane_img_data(path) #img_data的数据类型与大小？
...
def eval_predictions(pred_dir,
    ...
    predictions = load_culane_data(pred_dir, list_path) #每张图一个prediction
    annotations = load_culane_data(anno_dir, list_path) #每张图一个annotation
    img_shape = (590, 1640, 3)
    if sequential:
        results = map(
