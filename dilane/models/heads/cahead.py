...
from dilane.models.utils.roi_gather import LinearModule
...
from dilane.models.utils.dynamic_head import DynamicHead
...
class CAHead(nn.Module):
    def __init__(self,
        ...
        self.train_stage = self.refine_layers - 2
        ...
        self.DyHead = DynamicHead(n=192, feat_size=36, inplanes=self.reg_hidden_dim, early_return=False)
    ...
    def _init_learnable_anchor(self):
        # normalize all of [start_y, start_x, theta] 
        self.prior_embeddings = nn.Embedding(self.num_priors, 3)

        bottom_priors_nums = self.num_priors * 3 // 4 #144
        left_priors_nums = self.num_priors // 8 #24
    ...
    def forward(self, x, **kwargs):
        ...
        priors = self.priors.repeat(batch_size, 1, 1) #[24, 192, 78] 
        priors_on_featmap = self.priors_on_featmap.repeat(batch_size, 1, 1) #[24, 192, 36]

        predictions_lists = []

        prior_features_stages = []
        for stage in range(self.refine_layers): #对每层特征图进行refine
        ...
        L3_feat = prior_features_stages[0].clone().transpose(2, 3) #[24, 192, 36, 64]
        a_features = prior_features_stages[0].contiguous().view(batch_size * self.num_priors, -1) #[24*192, 64*36]
        a_features = F.relu(self.fc_norm(self.fc(a_features))) #[24*192, 64]
        a_features = a_features.view(batch_size, self.num_priors, -1).transpose(0,1) #[24, 192, 64]

        c_features = prior_features_stages[2].clone().view(batch_size, self.num_priors, -1) #low-level

        decode_feat_g = self.transformer_Dec(tgt=a_features, memory=a_features).transpose(0, 1) #[192, 24, 64]
        decode_feat_m = self.DyHead(c_features, L3_feat)

        decode_feat = torch.cat([decode_feat_m, decode_feat_g], dim=-1) #24, 192, 128
