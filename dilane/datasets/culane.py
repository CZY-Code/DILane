...
import dilane.utils.culane_metric as culane_metric
import cv2
    ...
    def evaluate(self, predictions, output_basedir):
        ...
        for idx, pred in enumerate(predictions):
            ...
            if len(lanes) - len(pred) in [1, 2, 3, 4]:
                loss_lines[len(lanes) - len(pred) - 1].append(self.data_infos[idx]['img_name'])

            with open(os.path.join(output_dir, output_filename),'w') as out_file:
                out_file.write(output)

        for i, names in enumerate(loss_lines):
            with open(os.path.join(output_basedir,'loss_{}_lines.txt'.format(i + 1)), 'w') as f:
                for name in names:
                    f.write(name + '\n')
        ...
        result = culane_metric.eval_predictions(output_basedir,
                                                self.data_root,
                                                self.list_path,
                                                iou_thresholds=[0.5], # np.linspace(0.5, 0.95, 10)
                                                official=True)
